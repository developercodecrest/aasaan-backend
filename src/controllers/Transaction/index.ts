...
export const createTransaction = async (
    ...
    if (!orderId || !Types.ObjectId.isValid(orderId)) {
        respondWithError(res, 400, "A valid orderId is required.");
        return;
    }
...
export const listTransactions = async (
    ...
    if (orderId) {
        if (!Types.ObjectId.isValid(orderId)) {
            respondWithError(res, 400, "Invalid orderId supplied.");
            return;
        }
    ...
    try {
        const transactions = await Transaction.find(filter as any)
            ...
            .skip(skip)
            .limit(limitNumber);
        const total = await Transaction.countDocuments(filter as any);
